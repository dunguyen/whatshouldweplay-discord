...
export async function getOwnedSteamGames(
    steamid: string
): Promise<{ steamAppIds: { appId: number; playtime: number }[]; success: boolean; id: string }> {
    return new Promise<{ steamAppIds: { appId: number; playtime: number }[]; success: boolean; id: string }>(
        (resolve, reject) => {
            axios
                .get(
                    ...
                    if (response.status === 200 && response.data.response.games) {
                        resolve({
                            steamAppIds: response.data.response.games.map(
                                (game: { appid: string; playtime_forever: string }) => {
                                    return {
                                        appId: parseInt(game.appid, 10),
                                        playtime: parseInt(game.playtime_forever, 10),
                                    };
                                }
                            ),
                            success: true,
